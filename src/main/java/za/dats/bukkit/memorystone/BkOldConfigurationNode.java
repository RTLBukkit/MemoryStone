...
public class BkOldConfigurationNode {
	protected Map<String, Object> root;

	public BkOldConfigurationNode(Map<String, Object> root) {
		this.root = root;
	}

	/**
	 * Gets all of the cofiguration values within the Node as a key value pair,
	 * with the key being the full path and the value being the Object that is
	 * at the path.
	 * 
	 * @return A map of key value pairs with the path as the key and the object
	 *         as the value
	 */
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<BkOldConfigurationNode> getNodeList(String path, List<BkOldConfigurationNode> def) {
		List<Object> raw = getList(path);

		if (raw == null) {
			return def != null ? def : new ArrayList<BkOldConfigurationNode>();
		}

		List<BkOldConfigurationNode> list = new ArrayList<BkOldConfigurationNode>();

		for (Object o : raw) {
			if (o instanceof Map) {
				list.add(new BkOldConfigurationNode((Map<String, Object>) o));
			}
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public BkOldConfigurationNode getNode(String path) {
		Object raw = getProperty(path);

		if (raw instanceof Map) {
			return new BkOldConfigurationNode((Map<String, Object>) raw);
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Map<String, BkOldConfigurationNode> getNodes(String path) {
		Object o = getProperty(path);

		if (o == null) {
			return null;
		} else if (o instanceof Map) {
			Map<String, BkOldConfigurationNode> nodes = new HashMap<String, BkOldConfigurationNode>();

			for (Map.Entry<String, Object> entry : ((Map<String, Object>) o).entrySet()) {
				if (entry.getValue() instanceof Map) {
					nodes.put(entry.getKey(), new BkOldConfigurationNode((Map<String, Object>) entry.getValue()));
				}
			}
